...
PATH = "/kaggle/working/progetto/GLASS"
...
def buildModel(hidden_dim, conv_layer, dropout, jk, pool, z_ratio, aggr):
    ...
    if args.use_nodeid:
        print("load ", f"{PATH}/Emb/{args.dataset}_{hidden_dim}.pt")
        emb = torch.load(f"{PATH}/Emb/{args.dataset}_{hidden_dim}.pt",
...
print(args)
# read configuration
with open(f"{PATH}/config/{args.dataset}.yml") as f:
